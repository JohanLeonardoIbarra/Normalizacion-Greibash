class Modelo {
  ...
  eliminarInalcanzables(inicial) {
    ...
    let eliminadas = this.alfabeto.filter((x) => !depuradas.join("").includes(x));
    eliminadas.forEach(
        (eliminada) => {
  ...
  eliminarInalcanzablesR(inicial, anteriores, otras) {
    ...
    let producciones = this.producciones[this.alfabeto.indexOf(inicial)].split("").concat(otras).filter(x => (x != "/")&&(x != inicial)&&(!anteriores.some(z => x == z)));
    ...
    return this.eliminarInalcanzablesR(producciones[0], anteriores, producciones);
    }
    ...
    eliminarRepetidas(){
        this.producciones = this.producciones.map(produccion => {
            return produccion.split("/").reduce((array, x) => {
                if(!array.includes(x)){
                    array.push(x);
                }
                return array;
...
const model = new Modelo(
  ["A", "B", "Z", "C", "D"],
  ["AB/DC/B/E/F/G/AED", "CD/EC/λ", "AD/CDA/DX/FG/λ", "ABC/CDG/A/λ", "AB/CD/XS/H"],
  [1, 2, 3, 4]
);
